...
namespace PN {
	...
	void fixIt(LT::LexTable& lextable, const std::string& str, size_t length, size_t pos, const std::vector<int>& ids) {
		for (size_t i = 0, q = 0; i < str.size(); i++) {
			lextable.table[pos + i].lexema[0] = str[i];
			if (lextable.table[pos + i].lexema[0] == LEX_ID || lextable.table[pos + i].lexema[0] == LEX_LITERAL|| lextable.table[pos + i].lexema[0]==LEX_EXTERN_FUNCTION) {
		...
		for (size_t i = 0; i < length - str.size(); i++) {
			...
			lextable.table[temp + i].lexema[0] = '!';
	...
	bool NotaciaPolska(int lextable_pos, LT::LexTable& lextable, IT::IdTable& idtable)
	{
		...
		for (int i = lextable_pos; i < lextable.size; i++, iterator++) {
			char lexem = lextable.table[i].lexema[0];
			char data = lextable.table[i].lexema[0];
			size_t stack_size = stack.size();
			if (idtable.table[lextable.table[i].idxTI].idtype == IT::IDTYPE::F|| 
			idtable.table[lextable.table[i].idxTI].idtype == IT::IDTYPE::EF) {
				stack.push('@');
				operands_count--;
			}
			switch (lexem) {
	...
	void Wykonac(LT::LexTable &lextable,IT::IdTable &idtable) {
		for (int i = 0; i <lextable.size; i++)
			if (lextable.table[i].lexema[0] == LEX_EQUAL)
				if (!NotaciaPolska(i + 1, lextable, idtable))
